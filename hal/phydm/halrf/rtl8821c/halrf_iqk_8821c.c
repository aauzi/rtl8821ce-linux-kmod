				   ...
				   /*1: reload TX, 2: reload TX, RX*/)
{
	...
	for (path = 0; path < SS_8821C; path++) {
		...
		for (idx = 0; idx < reload_idx; idx++) {
			...
			if (idx == 0)
				odm_set_bb_reg(dm, iqk_apply[path], BIT(0), (iqk_info->iqk_fail_report[channel][path][idx]) ? 0 : 1);
			else
				odm_set_bb_reg(dm, iqk_apply[path], BIT(10), (iqk_info->iqk_fail_report[channel][path][idx]) ? 0 : 1);
		}
